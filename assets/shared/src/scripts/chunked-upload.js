...
(function () {
    ...
    function toggleLoadingMessage($form) {
        ...
        return function () {
            if (!$form.find('.vimeify-uploading-in-progress-alert').length) {
                $form.find('.vimeify-submit-container').before('<div class="vimeify-error-alert vimeify-uploading-in-progress-alert">' + window.Vimeify_Chunked_Upload.loading_message + '</div>');
            }
        };
    }
    ...
    function toggleSubmit(dz) {
        ...
        if (disabled) {
            ...
            if (!$form.find('.vimeify-submit-overlay').length) {
                $btn.parent().addClass('vimeify-submit-overlay-container');
                $btn.parent().append('<div class="vimeify-submit-overlay"></div>');
                $form.find('.vimeify-submit-overlay').css('width', $btn.outerWidth() + 'px');
                $form.find('.vimeify-submit-overlay').css('height', $btn.parent().outerHeight() + 'px');
                $form.find('.vimeify-submit-overlay').on('click', handler);
            }
        } else {
            $btn.prop('disabled', false);
            $form.find('.vimeify-submit-overlay').off('click', handler);
            $form.find('.vimeify-submit-overlay').remove();
            $btn.parent().removeClass('vimeify-submit-overlay-container');
            if ($form.find('.vimeify-uploading-in-progress-alert').length) {
                $form.find('.vimeify-uploading-in-progress-alert').remove();
            }
        }
    }
    ...
    function updateInputValue(dz) {
        ...
        var $input = jQuery(dz.element).closest('.vimeify-chunked-uploader-wrapper').find('input[name=' + dz.dataTransfer.name + ']');
    ...
    function confirmChunksFinishUpload(dz) {
        ...
        return function confirm(file) {
            ...
            function retry() {
                ...
                if (file.retries === 3) {
                    addErrorMessage(file, window.Vimeify_Chunked_Upload.errors.file_not_uploaded);
                    return;
                }
    ...
    function validatePostMaxSizeError(file, dz) {
        ...
        setTimeout(function () {
            if (file.size >= dz.dataTransfer.postMaxSize) {
                var errorMessage = window.Vimeify_Chunked_Upload.errors.post_max_size;
                if (!file.isErrorNotUploadedDisplayed) {
                    file.isErrorNotUploadedDisplayed = true;
                    errorMessage = window.Vimeify_Chunked_Upload.errors.file_not_uploaded + ' ' + errorMessage;
    ...
    function error(dz) {
        ...
        return function (file, errorMessage) {
            ...
            file.isErrorNotUploadedDisplayed = true;
            file.previewElement.querySelectorAll('[data-dz-errormessage]')[0].textContent = window.Vimeify_Chunked_Upload.errors.file_not_uploaded + ' ' + errorMessage;
        };
    }
    ...
    function dropZoneInit($el) {
        ...
        var dz = new window.Dropzone($el, {
            url: window.Vimeify_Chunked_Upload.url,
            addRemoveLinks: true,
            chunking: true,
            forceChunking: true,
            retryChunks: true,
            ...
            dictMaxFilesExceeded: window.Vimeify_Chunked_Upload.errors.file_limit.replace('{fileLimit}', maxFiles),
            dictInvalidFileType: window.Vimeify_Chunked_Upload.errors.file_extension,
            dictFileTooBig: window.Vimeify_Chunked_Upload.errors.file_size,
        });
    ...
    function ready() {
        ...
        window.vimeify_chunked_upload_fields.dropzones = [].slice.call(document.querySelectorAll('.vimeify-chunked-uploader')).map(dropZoneInit);
    }

    /**
     * File uploader init.
     * @type {{init: vimeifyChunkedFileUpload.init}}
     */
    var vimeifyChunkedFileUpload = {
    ...
    // Initialize.
    vimeifyChunkedFileUpload.init();
    window.vimeifyChunkedFileUpload = vimeifyChunkedFileUpload;
}());
