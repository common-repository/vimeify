...
class Ajax extends BaseProvider {
	...
	public function handle_delete() {
		...
		if ( ! isset( $_POST['vimeo_uri'] ) || empty( $_POST['vimeo_uri'] ) ) {
			...
			if ( isset( $_POST['post_id'] ) && is_numeric( $_POST['post_id'] ) ) {
				...
				if ( $deleted ) {
					$args['title']        = __( 'Success', 'vimeify' );
					$args['message']      = __( 'Video deleted successfully.', 'vimeify' );
					$args['local_delete'] = 1;
	...
	public function get_uploads() {
		...
		$current_user_uploads = ! current_user_can( 'administrator' ) && (int) $this->plugin->system()->settings()->plugin()->get( 'admin.tinymce.show_author_uploads_only' );
	...
	public function handle_generate_stats() {
		...
		$author_uplaods_only = (int) $this->plugin->system()->settings()->plugin()->get( 'admin.videos_list_table.show_author_uploads_only' );
