...
class PostTypes extends BaseProvider {
	...
	public function register_vimeo_uplaods() {
		...
		$args   = array(
			...
			'publicly_queryable'  => ( (bool) $this->plugin->system()->settings()->plugin()->get( 'frontend.behavior.enable_single_pages' ) ) ? true : false,
			'rewrite'             => true,
			'capability_type'     => 'page',
		);

		$post_slug = apply_filters( 'vimeify_post_type_slug', 'vimeo-upload' );
		if ( ! empty( $post_slug ) ) {
			$args['rewrite'] = array(
				'slug'       => $post_slug,
				'with_front' => true,
		...
		register_post_type( Database::POST_TYPE_UPLOADS, apply_filters( 'vimeify_post_type_args', $args ) );
		...
		register_taxonomy( Database::TAX_CATEGORY, array( 'vimeify-upload' ), $args );
	}
